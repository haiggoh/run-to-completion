...
1. **Ask blocking questions up front.** Before starting, surface any genuinely blocking, matter-of-taste question you can already foresee (naming, structural choices, scope boundaries) — get those resolved once, before execution begins. If the plan will end in a push, publish, or repo-create (anything that would normally need the destructive/public-action confirmation below), ask for that permission's scope as part of this same up-front pass — don't wait until you hit it late in the run and stall the loop.
