...
1. **Ask blocking questions up front.** (Attended runs differ from unattended ones here: because the user is *present*, prefer surfacing and clearing gates as early as possible — an answer is cheap now and unblocks work immediately. An unattended `autopilot` run does the opposite, asking only about the run itself and turning per-item questions into gates. Same plugin, opposite instinct, decided by whether anyone is there to answer.) Before starting, surface any genuinely blocking, matter-of-taste question you can already foresee (naming, structural choices, scope boundaries) — get those resolved once, before execution begins. If the plan will end in a push, tag, release, publish, or repo-create (anything that would normally need the destructive/public-action confirmation below), ask for that permission's scope as part of this same up-front pass — and ask it as a **depth**, since “you may push” does not settle whether you may also tag and cut a release. Don't wait until you hit it late in the run and stall the loop.
